...
DOCKERFILE="${DOTNET_DOCKERFILE:-apps/otel-dotnet/Dockerfile}"

BUILD_CONTEXT="${DOTNET_BUILD_CONTEXT:-apps/otel-dotnet}"

TARGET_PLATFORM="${DOTNET_TARGET_PLATFORM:-linux/amd64}"
...
if [[ "$IMAGE_PLATFORM" != "$TARGET_PLATFORM" ]]; then
    fail "Image platform mismatch. Expected: $TARGET_PLATFORM, Found: $IMAGE_PLATFORM"
fi
...
export DOTNET_IMAGE_NAME="$IMAGE_NAME"
export DOTNET_IMAGE_TAG="$IMAGE_TAG"
export AZURE_DOTNET_IMAGE="$IMAGE_REFERENCE"
